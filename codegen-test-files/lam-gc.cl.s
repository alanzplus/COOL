# start of generated code

# coding global data 
	.data
	.align	2
	.globl	class_nameTab
	.globl	Main_protObj
	.globl	Int_protObj
	.globl	String_protObj
	.globl	bool_const0
	.globl	bool_const1
	.globl	_int_tag
	.globl	_bool_tag
	.globl	_string_tag
_int_tag:
	.word	10
_bool_tag:
	.word	11
_string_tag:
	.word	12

# choosing gc
	.globl	_MemMgr_INITIALIZER
_MemMgr_INITIALIZER:
	.word	_GenGC_Init
	.globl	_MemMgr_COLLECTOR
_MemMgr_COLLECTOR:
	.word	_GenGC_Collect
	.globl	_MemMgr_TEST
_MemMgr_TEST:
	.word	0

# coding constants
	.word	-1
str_const75:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const1
	.byte	0	
	.align	2
	.word	-1
str_const74:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const2
	.ascii	"Main"
	.byte	0	
	.align	2
	.word	-1
str_const73:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const2
	.ascii	"Term"
	.byte	0	
	.align	2
	.word	-1
str_const72:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const3
	.ascii	"App"
	.byte	0	
	.align	2
	.word	-1
str_const71:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const4
	.ascii	"Lambda"
	.byte	0	
	.align	2
	.word	-1
str_const70:
	.word	12
	.word	7
	.word	String_dispatch_tab
	.word	int_const5
	.ascii	"Variable"
	.byte	0	
	.align	2
	.word	-1
str_const69:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const2
	.ascii	"Expr"
	.byte	0	
	.align	2
	.word	-1
str_const68:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"LambdaListRef"
	.byte	0	
	.align	2
	.word	-1
str_const67:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const7
	.ascii	"LambdaListNE"
	.byte	0	
	.align	2
	.word	-1
str_const66:
	.word	12
	.word	7
	.word	String_dispatch_tab
	.word	int_const8
	.ascii	"LambdaList"
	.byte	0	
	.align	2
	.word	-1
str_const65:
	.word	12
	.word	7
	.word	String_dispatch_tab
	.word	int_const9
	.ascii	"VarListNE"
	.byte	0	
	.align	2
	.word	-1
str_const64:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const10
	.ascii	"VarList"
	.byte	0	
	.align	2
	.word	-1
str_const63:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const4
	.ascii	"String"
	.byte	0	
	.align	2
	.word	-1
str_const62:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const2
	.ascii	"Bool"
	.byte	0	
	.align	2
	.word	-1
str_const61:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const3
	.ascii	"Int"
	.byte	0	
	.align	2
	.word	-1
str_const60:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const11
	.ascii	"IO"
	.byte	0	
	.align	2
	.word	-1
str_const59:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const4
	.ascii	"Object"
	.byte	0	
	.align	2
	.word	-1
str_const58:
	.word	12
	.word	7
	.word	String_dispatch_tab
	.word	int_const8
	.ascii	"_prim_slot"
	.byte	0	
	.align	2
	.word	-1
str_const57:
	.word	12
	.word	7
	.word	String_dispatch_tab
	.word	int_const9
	.ascii	"SELF_TYPE"
	.byte	0	
	.align	2
	.word	-1
str_const56:
	.word	12
	.word	7
	.word	String_dispatch_tab
	.word	int_const9
	.ascii	"_no_class"
	.byte	0	
	.align	2
	.word	-1
str_const55:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"<basic class>"
	.byte	0	
	.align	2
	.word	-1
str_const54:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const10
	.ascii	"\n};\n};\n"
	.byte	0	
	.align	2
	.word	-1
str_const53:
	.word	12
	.word	11
	.word	String_dispatch_tab
	.word	int_const12
	.ascii	"  main() : EvalObject {\n"
	.byte	0	
	.align	2
	.word	-1
str_const52:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"class Main {\n"
	.byte	0	
	.align	2
	.word	-1
str_const51:
	.word	12
	.word	17
	.word	String_dispatch_tab
	.word	int_const13
	.ascii	"(*Generated by lam.cl (Jeff Foster, March 2000)*)\n"
	.byte	0	
	.align	2
	.word	-1
str_const50:
	.word	12
	.word	16
	.word	String_dispatch_tab
	.word	int_const14
	.ascii	"\n------------------cut here------------------\n"
	.byte	0	
	.align	2
	.word	-1
str_const49:
	.word	12
	.word	10
	.word	String_dispatch_tab
	.word	int_const15
	.ascii	"Generating code for "
	.byte	0	
	.align	2
	.word	-1
str_const48:
	.word	12
	.word	20
	.word	String_dispatch_tab
	.word	int_const16
	.ascii	"  apply(y : EvalObject) : EvalObject { { abort(); self; } };\n"
	.byte	0	
	.align	2
	.word	-1
str_const47:
	.word	12
	.word	19
	.word	String_dispatch_tab
	.word	int_const17
	.ascii	"  init(p : Closure) : Closure {{ parent <- p; self; }};\n"
	.byte	0	
	.align	2
	.word	-1
str_const46:
	.word	12
	.word	12
	.word	String_dispatch_tab
	.word	int_const18
	.ascii	"  get_x() : EvalObject { x };\n"
	.byte	0	
	.align	2
	.word	-1
str_const45:
	.word	12
	.word	14
	.word	String_dispatch_tab
	.word	int_const19
	.ascii	"  get_parent() : Closure { parent };\n"
	.byte	0	
	.align	2
	.word	-1
str_const44:
	.word	12
	.word	9
	.word	String_dispatch_tab
	.word	int_const20
	.ascii	"  x : EvalObject;\n"
	.byte	0	
	.align	2
	.word	-1
str_const43:
	.word	12
	.word	10
	.word	String_dispatch_tab
	.word	int_const15
	.ascii	"  parent : Closure;\n"
	.byte	0	
	.align	2
	.word	-1
str_const42:
	.word	12
	.word	14
	.word	String_dispatch_tab
	.word	int_const21
	.ascii	"class Closure inherits EvalObject {\n"
	.byte	0	
	.align	2
	.word	-1
str_const41:
	.word	12
	.word	16
	.word	String_dispatch_tab
	.word	int_const14
	.ascii	"  eval() : EvalObject { { abort(); self; } };\n"
	.byte	0	
	.align	2
	.word	-1
str_const40:
	.word	12
	.word	12
	.word	String_dispatch_tab
	.word	int_const22
	.ascii	"class EvalObject inherits IO {\n"
	.byte	0	
	.align	2
	.word	-1
str_const39:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const2
	.ascii	" =>\n"
	.byte	0	
	.align	2
	.word	-1
str_const38:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"beta-reduce: "
	.byte	0	
	.align	2
	.word	-1
str_const37:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const0
	.ascii	"z"
	.byte	0	
	.align	2
	.word	-1
str_const36:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const0
	.ascii	"y"
	.byte	0	
	.align	2
	.word	-1
str_const35:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const0
	.ascii	"x"
	.byte	0	
	.align	2
	.word	-1
str_const34:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const10
	.ascii	"  esac)"
	.byte	0	
	.align	2
	.word	-1
str_const33:
	.word	12
	.word	17
	.word	String_dispatch_tab
	.word	int_const23
	.ascii	"    o : Object => { abort(); new EvalObject; };\n"
	.byte	0	
	.align	2
	.word	-1
str_const32:
	.word	12
	.word	12
	.word	String_dispatch_tab
	.word	int_const22
	.ascii	"    c : Closure => c.apply(y);\n"
	.byte	0	
	.align	2
	.word	-1
str_const31:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const7
	.ascii	"  case x of\n"
	.byte	0	
	.align	2
	.word	-1
str_const30:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const2
	.ascii	" in\n"
	.byte	0	
	.align	2
	.word	-1
str_const29:
	.word	12
	.word	10
	.word	String_dispatch_tab
	.word	int_const24
	.ascii	"     y : EvalObject <- "
	.byte	0	
	.align	2
	.word	-1
str_const28:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const11
	.ascii	",\n"
	.byte	0	
	.align	2
	.word	-1
str_const27:
	.word	12
	.word	10
	.word	String_dispatch_tab
	.word	int_const24
	.ascii	"(let x : EvalObject <- "
	.byte	0	
	.align	2
	.word	-1
str_const26:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const11
	.ascii	"))"
	.byte	0	
	.align	2
	.word	-1
str_const25:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const3
	.ascii	")@("
	.byte	0	
	.align	2
	.word	-1
str_const24:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const11
	.ascii	"(("
	.byte	0	
	.align	2
	.word	-1
str_const23:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const3
	.ascii	"};\n"
	.byte	0	
	.align	2
	.word	-1
str_const22:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const25
	.ascii	";}};\n"
	.byte	0	
	.align	2
	.word	-1
str_const21:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const26
	.ascii	"      x <- y;\n"
	.byte	0	
	.align	2
	.word	-1
str_const20:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const4
	.byte	92
	.ascii	"n\");\n"
	.byte	0	
	.align	2
	.word	-1
str_const19:
	.word	12
	.word	13
	.word	String_dispatch_tab
	.word	int_const27
	.ascii	"    { out_string(\"Applying closure "
	.byte	0	
	.align	2
	.word	-1
str_const18:
	.word	12
	.word	14
	.word	String_dispatch_tab
	.word	int_const28
	.ascii	"  apply(y : EvalObject) : EvalObject {\n"
	.byte	0	
	.align	2
	.word	-1
str_const17:
	.word	12
	.word	10
	.word	String_dispatch_tab
	.word	int_const15
	.ascii	" inherits Closure {\n"
	.byte	0	
	.align	2
	.word	-1
str_const16:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"class Closure"
	.byte	0	
	.align	2
	.word	-1
str_const15:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const4
	.ascii	"self))"
	.byte	0	
	.align	2
	.word	-1
str_const14:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"new Closure))"
	.byte	0	
	.align	2
	.word	-1
str_const13:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const10
	.ascii	").init("
	.byte	0	
	.align	2
	.word	-1
str_const12:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"((new Closure"
	.byte	0	
	.align	2
	.word	-1
str_const11:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const0
	.ascii	"."
	.byte	0	
	.align	2
	.word	-1
str_const10:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const0
	.byte	92
	.byte	0	
	.align	2
	.word	-1
str_const9:
	.word	12
	.word	6
	.word	String_dispatch_tab
	.word	int_const10
	.ascii	"get_x()"
	.byte	0	
	.align	2
	.word	-1
str_const8:
	.word	12
	.word	11
	.word	String_dispatch_tab
	.word	int_const29
	.ascii	"Error:  free occurrence of "
	.byte	0	
	.align	2
	.word	-1
str_const7:
	.word	12
	.word	8
	.word	String_dispatch_tab
	.word	int_const6
	.ascii	"get_parent()."
	.byte	0	
	.align	2
	.word	-1
str_const6:
	.word	12
	.word	16
	.word	String_dispatch_tab
	.word	int_const30
	.ascii	"\nError: Expr is pure virtual; can't gen_code\n"
	.byte	0	
	.align	2
	.word	-1
str_const5:
	.word	12
	.word	16
	.word	String_dispatch_tab
	.word	int_const31
	.ascii	"\nError: Expr is pure virtual; can't substitute\n"
	.byte	0	
	.align	2
	.word	-1
str_const4:
	.word	12
	.word	17
	.word	String_dispatch_tab
	.word	int_const23
	.ascii	"\nError: Expr is pure virtual; can't beta-reduce\n"
	.byte	0	
	.align	2
	.word	-1
str_const3:
	.word	12
	.word	16
	.word	String_dispatch_tab
	.word	int_const31
	.ascii	"\nError: Expr is pure virtual; can't print self\n"
	.byte	0	
	.align	2
	.word	-1
str_const2:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const0
	.ascii	" "
	.byte	0	
	.align	2
	.word	-1
str_const1:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const0
	.ascii	"\n"
	.byte	0	
	.align	2
	.word	-1
str_const0:
	.word	12
	.word	7
	.word	String_dispatch_tab
	.word	int_const32
	.ascii	"./lam-gc.cl"
	.byte	0	
	.align	2
	.word	-1
int_const32:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	11
	.word	-1
int_const31:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	47
	.word	-1
int_const30:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	45
	.word	-1
int_const29:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	27
	.word	-1
int_const28:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	39
	.word	-1
int_const27:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	35
	.word	-1
int_const26:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	14
	.word	-1
int_const25:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	5
	.word	-1
int_const24:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	23
	.word	-1
int_const23:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	48
	.word	-1
int_const22:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	31
	.word	-1
int_const21:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	36
	.word	-1
int_const20:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	18
	.word	-1
int_const19:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	37
	.word	-1
int_const18:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	30
	.word	-1
int_const17:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	56
	.word	-1
int_const16:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	61
	.word	-1
int_const15:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	20
	.word	-1
int_const14:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	46
	.word	-1
int_const13:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	50
	.word	-1
int_const12:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	24
	.word	-1
int_const11:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	2
	.word	-1
int_const10:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	7
	.word	-1
int_const9:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	9
	.word	-1
int_const8:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	10
	.word	-1
int_const7:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	12
	.word	-1
int_const6:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	13
	.word	-1
int_const5:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	8
	.word	-1
int_const4:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	6
	.word	-1
int_const3:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	3
	.word	-1
int_const2:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	4
	.word	-1
int_const1:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	0
	.word	-1
int_const0:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	1
	.word	-1
bool_const0:
	.word	11
	.word	4
	.word	Bool_dispatch_tab
	.word	0
	.word	-1
bool_const1:
	.word	11
	.word	4
	.word	Bool_dispatch_tab
	.word	1

# Declare Class Tag
	.globl	_App_tag
	.globl	_Expr_tag
	.globl	_IO_tag
	.globl	_Lambda_tag
	.globl	_LambdaList_tag
	.globl	_LambdaListNE_tag
	.globl	_LambdaListRef_tag
	.globl	_Main_tag
	.globl	_Object_tag
	.globl	_Term_tag
	.globl	_VarList_tag
	.globl	_VarListNE_tag
	.globl	_Variable_tag

# Class Tag Definition
_App_tag:
	.word	7
_Expr_tag:
	.word	4
_IO_tag:
	.word	1
_Lambda_tag:
	.word	6
_LambdaList_tag:
	.word	13
_LambdaListNE_tag:
	.word	14
_LambdaListRef_tag:
	.word	15
_Main_tag:
	.word	9
_Object_tag:
	.word	0
_Term_tag:
	.word	8
_VarList_tag:
	.word	2
_VarListNE_tag:
	.word	3
_Variable_tag:
	.word	5

# Class Name String Table
class_nameTab:
	.word	str_const59
	.word	str_const60
	.word	str_const64
	.word	str_const65
	.word	str_const69
	.word	str_const70
	.word	str_const71
	.word	str_const72
	.word	str_const73
	.word	str_const74
	.word	str_const61
	.word	str_const62
	.word	str_const63
	.word	str_const66
	.word	str_const67
	.word	str_const68

# Class Dispatch Table
App_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	App.print_self
	.word	App.beta
	.word	App.substitute
	.word	App.gen_code
	.word	App.init

Bool_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy

Expr_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	Expr.print_self
	.word	Expr.beta
	.word	Expr.substitute
	.word	Expr.gen_code

IO_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int

Int_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy

Lambda_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	Lambda.print_self
	.word	Lambda.beta
	.word	Lambda.substitute
	.word	Lambda.gen_code
	.word	Lambda.init
	.word	Lambda.apply
	.word	Lambda.gen_closure_code

LambdaList_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	LambdaList.isNil
	.word	LambdaList.headE
	.word	LambdaList.headC
	.word	LambdaList.headN
	.word	LambdaList.tail
	.word	LambdaList.add

LambdaListNE_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	LambdaListNE.isNil
	.word	LambdaListNE.headE
	.word	LambdaListNE.headC
	.word	LambdaListNE.headN
	.word	LambdaListNE.tail
	.word	LambdaList.add
	.word	LambdaListNE.init

LambdaListRef_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	LambdaListRef.isNil
	.word	LambdaListRef.headE
	.word	LambdaListRef.headC
	.word	LambdaListRef.headN
	.word	LambdaListRef.reset
	.word	LambdaListRef.add
	.word	LambdaListRef.removeHead

Main_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	Term.var
	.word	Term.lam
	.word	Term.app
	.word	Term.i
	.word	Term.k
	.word	Term.s
	.word	Main.beta_reduce
	.word	Main.eval_class
	.word	Main.closure_class
	.word	Main.gen_code
	.word	Main.main

Object_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy

String_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	String.length
	.word	String.concat
	.word	String.substr

Term_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	Term.var
	.word	Term.lam
	.word	Term.app
	.word	Term.i
	.word	Term.k
	.word	Term.s

VarList_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	VarList.isNil
	.word	VarList.head
	.word	VarList.tail
	.word	VarList.add
	.word	VarList.print

VarListNE_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	VarListNE.isNil
	.word	VarListNE.head
	.word	VarListNE.tail
	.word	VarList.add
	.word	VarListNE.print
	.word	VarListNE.init

Variable_dispatch_tab:
	.word	Object.abort
	.word	Object.type_name
	.word	Object.copy
	.word	IO.out_string
	.word	IO.out_int
	.word	IO.in_string
	.word	IO.in_int
	.word	Variable.print_self
	.word	Variable.beta
	.word	Variable.substitute
	.word	Variable.gen_code
	.word	Variable.init


# Class Prototype
	.word	-1
App_protObj:
	.word	7
	.word	5
	.word	App_dispatch_tab
	.word	0
	.word	0
	.word	-1
Bool_protObj:
	.word	11
	.word	4
	.word	Bool_dispatch_tab
	.word	0
	.word	-1
Expr_protObj:
	.word	4
	.word	3
	.word	Expr_dispatch_tab
	.word	-1
IO_protObj:
	.word	1
	.word	3
	.word	IO_dispatch_tab
	.word	-1
Int_protObj:
	.word	10
	.word	4
	.word	Int_dispatch_tab
	.word	0
	.word	-1
Lambda_protObj:
	.word	6
	.word	5
	.word	Lambda_dispatch_tab
	.word	0
	.word	0
	.word	-1
LambdaList_protObj:
	.word	13
	.word	3
	.word	LambdaList_dispatch_tab
	.word	-1
LambdaListNE_protObj:
	.word	14
	.word	7
	.word	LambdaListNE_dispatch_tab
	.word	0
	.word	int_const1
	.word	0
	.word	0
	.word	-1
LambdaListRef_protObj:
	.word	15
	.word	5
	.word	LambdaListRef_dispatch_tab
	.word	int_const1
	.word	0
	.word	-1
Main_protObj:
	.word	9
	.word	3
	.word	Main_dispatch_tab
	.word	-1
Object_protObj:
	.word	0
	.word	3
	.word	Object_dispatch_tab
	.word	-1
String_protObj:
	.word	12
	.word	5
	.word	String_dispatch_tab
	.word	int_const1
	.word	0
	.word	-1
Term_protObj:
	.word	8
	.word	3
	.word	Term_dispatch_tab
	.word	-1
VarList_protObj:
	.word	2
	.word	3
	.word	VarList_dispatch_tab
	.word	-1
VarListNE_protObj:
	.word	3
	.word	5
	.word	VarListNE_dispatch_tab
	.word	0
	.word	0
	.word	-1
Variable_protObj:
	.word	5
	.word	4
	.word	Variable_dispatch_tab
	.word	str_const75

class_direct_parent_tab:
	.word	-1 #Object, tag_no:0, , tag_no:-1
	.word	0 #IO, tag_no:1, Object, tag_no:0
	.word	1 #VarList, tag_no:2, IO, tag_no:1
	.word	2 #VarListNE, tag_no:3, VarList, tag_no:2
	.word	1 #Expr, tag_no:4, IO, tag_no:1
	.word	4 #Variable, tag_no:5, Expr, tag_no:4
	.word	4 #Lambda, tag_no:6, Expr, tag_no:4
	.word	4 #App, tag_no:7, Expr, tag_no:4
	.word	1 #Term, tag_no:8, IO, tag_no:1
	.word	8 #Main, tag_no:9, Term, tag_no:8
	.word	0 #Int, tag_no:10, Object, tag_no:0
	.word	0 #Bool, tag_no:11, Object, tag_no:0
	.word	0 #String, tag_no:12, Object, tag_no:0
	.word	0 #LambdaList, tag_no:13, Object, tag_no:0
	.word	13 #LambdaListNE, tag_no:14, LambdaList, tag_no:13
	.word	0 #LambdaListRef, tag_no:15, Object, tag_no:0

class_prot_obj_tab:
	.word	Object_protObj
	.word	IO_protObj
	.word	VarList_protObj
	.word	VarListNE_protObj
	.word	Expr_protObj
	.word	Variable_protObj
	.word	Lambda_protObj
	.word	App_protObj
	.word	Term_protObj
	.word	Main_protObj
	.word	Int_protObj
	.word	Bool_protObj
	.word	String_protObj
	.word	LambdaList_protObj
	.word	LambdaListNE_protObj
	.word	LambdaListRef_protObj

class_init_tab:
	.word	Object_init
	.word	IO_init
	.word	VarList_init
	.word	VarListNE_init
	.word	Expr_init
	.word	Variable_init
	.word	Lambda_init
	.word	App_init
	.word	Term_init
	.word	Main_init
	.word	Int_init
	.word	Bool_init
	.word	String_init
	.word	LambdaList_init
	.word	LambdaListNE_init
	.word	LambdaListRef_init

	.globl	heap_start
heap_start:
	.word	0

	.text
	.globl	Main_init
	.globl	Int_init
	.globl	String_init
	.globl	Bool_init
	.globl	Main.main

# Class Init Method
App_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Expr_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Bool_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Object_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Expr_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	IO_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
IO_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Object_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Int_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Object_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Lambda_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Expr_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
LambdaList_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Object_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
LambdaListNE_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	LambdaList_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
LambdaListRef_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Object_init
	la	$a0 int_const1
	sw	$a0	12($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Main_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Term_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Object_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
String_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Object_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Term_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	IO_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
VarList_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	IO_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
VarListNE_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	VarList_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra
Variable_init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	jal	Expr_init
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	jr	$ra

# Class Methods 

App.init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	16($fp)
	sw	$a0	12($s0)
	lw	$a0	12($fp)
	sw	$a0	16($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	8
	jr	$ra

App.print_self:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const24
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label0
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label0:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	12($s0)
	bne	$a0 $zero label1
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label1:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const25
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label2
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label2:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label3
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label3:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const26
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label4
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label4:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

App.beta:
	addiu	$sp	$sp	-24
	sw	$fp	24($sp)
	sw	$s0	20($sp)
	sw	$ra	16($sp)
	addiu	$fp	$sp	16
	move	$s0	$a0
	lw	$a0	12($s0)
	bne	$a0 $zero label5
	la	$a0 str_const0
	li	$t1 1
	jal	_case_abort2
label5:
	lw	$t1	0($a0)
# find ancestor
label8:
	li	$t2 6
	beq	$t1 $t2 label6
	li	$t2 4
	beq	$t1 $t2 label7
	li	$t2 0
	beq	$t1 $t2 label9
	la	$t2 class_direct_parent_tab
	li	$t3 4
	mul	$t1	$t1	$t3
	add	$t1	$t1	$t2
	lw	$t1	0($t1)
	b	label8
label9:
	jal	_case_abort
label6:
	sw	$a0	-4($fp)
# dispatch:apply
# push arg :0
	lw	$a0	16($s0)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label11
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label11:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:apply end
	b	label10
label7:
	sw	$a0	-4($fp)
# dispatch:beta
# evaluate dispatch expr
	lw	$a0	12($s0)
	bne	$a0 $zero label12
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label12:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:beta end
	sw	$a0	-8($fp)
	la	$a0 App_protObj
	jal	Object.copy
	jal	App_init
	sw	$a0	-12($fp)
# dispatch:init
# push arg :0
	lw	$a0	-8($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	16($s0)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-12($fp)
	bne	$a0 $zero label13
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label13:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:init end
	b	label10
label10:
	lw	$fp	24($sp)
	lw	$s0	20($sp)
	lw	$ra	16($sp)
	addiu	$sp	$sp	24
	addiu	$sp	$sp	0
	jr	$ra

App.substitute:
	addiu	$sp	$sp	-24
	sw	$fp	24($sp)
	sw	$s0	20($sp)
	sw	$ra	16($sp)
	addiu	$fp	$sp	16
	move	$s0	$a0
# dispatch:substitute
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	12($s0)
	bne	$a0 $zero label14
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label14:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:substitute end
	sw	$a0	-4($fp)
# dispatch:substitute
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label15
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label15:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:substitute end
	sw	$a0	-8($fp)
	la	$a0 App_protObj
	jal	Object.copy
	jal	App_init
	sw	$a0	-12($fp)
# dispatch:init
# push arg :0
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-8($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-12($fp)
	bne	$a0 $zero label16
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label16:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:init end
	lw	$fp	24($sp)
	lw	$s0	20($sp)
	lw	$ra	16($sp)
	addiu	$sp	$sp	24
	addiu	$sp	$sp	8
	jr	$ra

App.gen_code:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const27
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label17
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label17:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:gen_code
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	12($s0)
	bne	$a0 $zero label18
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label18:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:gen_code end
# dispatch:out_string
# push arg :0
	la	$a0 str_const28
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label19
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label19:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const29
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label20
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label20:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:gen_code
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label21
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label21:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:gen_code end
# dispatch:out_string
# push arg :0
	la	$a0 str_const30
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label22
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label22:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const31
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label23
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label23:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const32
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label24
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label24:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const33
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label25
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label25:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const34
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label26
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label26:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	8
	jr	$ra

Expr.print_self:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const3
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label27
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label27:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label28
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label28:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Expr.beta:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const4
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label29
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label29:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label30
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label30:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Expr.substitute:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const5
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label31
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label31:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label32
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label32:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	8
	jr	$ra

Expr.gen_code:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const6
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label33
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label33:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label34
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label34:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	8
	jr	$ra

Lambda.init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	16($fp)
	sw	$a0	12($s0)
	lw	$a0	12($fp)
	sw	$a0	16($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	8
	jr	$ra

Lambda.print_self:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const10
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label35
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label35:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	12($s0)
	bne	$a0 $zero label36
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label36:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const11
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label37
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label37:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label38
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label38:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Lambda.beta:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Lambda.apply:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:substitute
# push arg :0
	lw	$a0	12($s0)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label39
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label39:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:substitute end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	4
	jr	$ra

Lambda.substitute:
	addiu	$sp	$sp	-20
	sw	$fp	20($sp)
	sw	$s0	16($sp)
	sw	$ra	12($sp)
	addiu	$fp	$sp	12
	move	$s0	$a0
	lw	$a0	16($fp)
	sw	$a0	-4($fp)
	lw	$a0	12($s0)
	sw	$a0	-8($fp)
	la	$a0	bool_const1
	lw	$t1	-4($fp)
	lw	$t2	-8($fp)
	beq	$t1 $t2 label40
	la	$a1	bool_const0
	jal	equality_test
label40:
	lw	$t1	12($a0)
	beqz	$t1 label41
	move	$a0	$s0
	b	label42
label41:
# dispatch:substitute
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label43
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label43:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:substitute end
	sw	$a0	-4($fp)
	la	$a0 Lambda_protObj
	jal	Object.copy
	jal	Lambda_init
	sw	$a0	-8($fp)
# dispatch:init
# push arg :0
	lw	$a0	12($s0)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-8($fp)
	bne	$a0 $zero label44
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label44:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:init end
label42:
	lw	$fp	20($sp)
	lw	$s0	16($sp)
	lw	$ra	12($sp)
	addiu	$sp	$sp	20
	addiu	$sp	$sp	8
	jr	$ra

Lambda.gen_code:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const12
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label45
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label45:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_int
# push arg :0
# dispatch:add
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	move	$a0	$s0
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	12($fp)
	bne	$a0 $zero label46
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label46:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:add end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label47
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label47:
	lw	$t1	8($a0)
	lw	$t1	16($t1)
	jalr	$t1
# dispatch:out_int end
# dispatch:out_string
# push arg :0
	la	$a0 str_const13
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label48
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label48:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:isNil
# evaluate dispatch expr
	lw	$a0	16($fp)
	bne	$a0 $zero label49
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label49:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:isNil end
	lw	$t1	12($a0)
	beqz	$t1 label50
# dispatch:out_string
# push arg :0
	la	$a0 str_const14
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label52
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label52:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	b	label51
label50:
# dispatch:out_string
# push arg :0
	la	$a0 str_const15
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label53
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label53:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
label51:
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	8
	jr	$ra

Lambda.gen_closure_code:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const16
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label54
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label54:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_int
# push arg :0
	lw	$a0	20($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label55
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label55:
	lw	$t1	8($a0)
	lw	$t1	16($t1)
	jalr	$t1
# dispatch:out_int end
# dispatch:out_string
# push arg :0
	la	$a0 str_const17
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label56
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label56:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const18
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label57
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label57:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const19
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label58
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label58:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_int
# push arg :0
	lw	$a0	20($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label59
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label59:
	lw	$t1	8($a0)
	lw	$t1	16($t1)
	jalr	$t1
# dispatch:out_int end
# dispatch:out_string
# push arg :0
	la	$a0 str_const20
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label60
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label60:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const21
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label61
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label61:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:gen_code
# push arg :0
# dispatch:add
# push arg :0
	lw	$a0	12($s0)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	16($fp)
	bne	$a0 $zero label62
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label62:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:add end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label63
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label63:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:gen_code end
# dispatch:out_string
# push arg :0
	la	$a0 str_const22
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label64
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label64:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const23
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label65
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label65:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	12
	jr	$ra

LambdaList.isNil:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	la	$a0 bool_const1
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaList.headE:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label66
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label66:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	la	$a0 VarList_protObj
	jal	Object.copy
	jal	VarList_init
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaList.headC:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label67
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label67:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	la	$a0 Lambda_protObj
	jal	Object.copy
	jal	Lambda_init
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaList.headN:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label68
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label68:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	la	$a0 int_const1
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaList.tail:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label69
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label69:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	la	$a0 LambdaList_protObj
	jal	Object.copy
	jal	LambdaList_init
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaList.add:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:init
# push arg :0
	lw	$a0	20($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :2
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :3
	move	$a0	$s0
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	la	$a0 LambdaListNE_protObj
	jal	Object.copy
	jal	LambdaListNE_init
	bne	$a0 $zero label70
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label70:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:init end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	12
	jr	$ra

LambdaListNE.isNil:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	la	$a0 bool_const0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListNE.headE:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	20($s0)
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListNE.headC:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	12($s0)
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListNE.headN:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	16($s0)
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListNE.tail:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	24($s0)
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListNE.init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	24($fp)
	sw	$a0	20($s0)
	lw	$a0	20($fp)
	sw	$a0	12($s0)
	lw	$a0	16($fp)
	sw	$a0	16($s0)
	lw	$a0	12($fp)
	sw	$a0	24($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	16
	jr	$ra

LambdaListRef.isNil:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:isNil
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label71
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label71:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:isNil end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListRef.headE:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:headE
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label72
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label72:
	lw	$t1	8($a0)
	lw	$t1	16($t1)
	jalr	$t1
# dispatch:headE end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListRef.headC:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:headC
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label73
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label73:
	lw	$t1	8($a0)
	lw	$t1	20($t1)
	jalr	$t1
# dispatch:headC end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListRef.headN:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:headN
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label74
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label74:
	lw	$t1	8($a0)
	lw	$t1	24($t1)
	jalr	$t1
# dispatch:headN end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListRef.reset:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	la	$a0 int_const1
	sw	$a0	12($s0)
	la	$a0 LambdaList_protObj
	jal	Object.copy
	jal	LambdaList_init
	sw	$a0	16($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

LambdaListRef.add:
	addiu	$sp	$sp	-20
	sw	$fp	20($sp)
	sw	$s0	16($sp)
	sw	$ra	12($sp)
	addiu	$fp	$sp	12
	move	$s0	$a0
# dispatch:add
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :2
	lw	$a0	12($s0)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label75
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label75:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:add end
	sw	$a0	16($s0)
	lw	$a0	12($s0)
	sw	$a0	-4($fp)
	la	$a0 int_const0
	sw	$a0	-8($fp)
	la	$a0 Int_protObj
	jal	Object.copy
	jal	Int_init
	lw	$t1	-4($fp)
	lw	$t1	12($t1)
	lw	$t2	-8($fp)
	lw	$t2	12($t2)
	add	$t1	$t1	$t2
	sw	$t1	12($a0)
	sw	$a0	12($s0)
	lw	$a0	12($s0)
	sw	$a0	-4($fp)
	la	$a0 int_const0
	sw	$a0	-8($fp)
	la	$a0 Int_protObj
	jal	Object.copy
	jal	Int_init
	lw	$t1	-4($fp)
	lw	$t1	12($t1)
	lw	$t2	-8($fp)
	lw	$t2	12($t2)
	sub	$t1	$t1	$t2
	sw	$t1	12($a0)
	lw	$fp	20($sp)
	lw	$s0	16($sp)
	lw	$ra	12($sp)
	addiu	$sp	$sp	20
	addiu	$sp	$sp	8
	jr	$ra

LambdaListRef.removeHead:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:tail
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label76
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label76:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:tail end
	sw	$a0	16($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Main.beta_reduce:
	addiu	$sp	$sp	-28
	sw	$fp	28($sp)
	sw	$s0	24($sp)
	sw	$ra	20($sp)
	addiu	$fp	$sp	20
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const38
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label77
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label77:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	12($fp)
	bne	$a0 $zero label78
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label78:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
	la	$a0 bool_const0
	sw	$a0	-4($fp)
	li	$a0	0
	sw	$a0	-8($fp)
# loop class
label79:
	lw	$a0	-4($fp)
	lw	$t1	12($a0)
	la	$a0	bool_const1
	beqz	$t1 label81
	la	$a0	bool_const0
label81:
	lw	$t1	12($a0)
	beqz	$t1 label80
# dispatch:beta
# evaluate dispatch expr
	lw	$a0	12($fp)
	bne	$a0 $zero label82
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label82:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:beta end
	sw	$a0	-8($fp)
	lw	$a0	-8($fp)
	sw	$a0	-12($fp)
	lw	$a0	12($fp)
	sw	$a0	-16($fp)
	la	$a0	bool_const1
	lw	$t1	-12($fp)
	lw	$t2	-16($fp)
	beq	$t1 $t2 label83
	la	$a1	bool_const0
	jal	equality_test
label83:
	lw	$t1	12($a0)
	beqz	$t1 label84
	la	$a0 bool_const1
	sw	$a0	-4($fp)
	b	label85
label84:
	lw	$a0	-8($fp)
	sw	$a0	12($fp)
# dispatch:out_string
# push arg :0
	la	$a0 str_const39
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label86
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label86:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	12($fp)
	bne	$a0 $zero label87
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label87:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
label85:
	b	label79
label80:
	li	$a0 0
# loop class end
# dispatch:out_string
# push arg :0
	la	$a0 str_const1
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label88
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label88:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$a0	12($fp)
	lw	$fp	28($sp)
	lw	$s0	24($sp)
	lw	$ra	20($sp)
	addiu	$sp	$sp	28
	addiu	$sp	$sp	4
	jr	$ra

Main.eval_class:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const40
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label89
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label89:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const41
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label90
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label90:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const23
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label91
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label91:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Main.closure_class:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const42
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label92
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label92:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const43
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label93
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label93:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const44
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label94
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label94:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const45
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label95
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label95:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const46
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label96
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label96:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const47
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label97
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label97:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const48
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label98
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label98:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const23
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label99
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label99:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Main.gen_code:
	addiu	$sp	$sp	-28
	sw	$fp	28($sp)
	sw	$s0	24($sp)
	sw	$ra	20($sp)
	addiu	$fp	$sp	20
	move	$s0	$a0
# dispatch:reset
# evaluate dispatch expr
	la	$a0 LambdaListRef_protObj
	jal	Object.copy
	jal	LambdaListRef_init
	bne	$a0 $zero label100
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label100:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:reset end
	sw	$a0	-4($fp)
# dispatch:out_string
# push arg :0
	la	$a0 str_const49
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label101
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label101:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	12($fp)
	bne	$a0 $zero label102
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label102:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const50
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label103
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label103:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const51
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label104
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label104:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:eval_class
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label105
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label105:
	lw	$t1	8($a0)
	lw	$t1	56($t1)
	jalr	$t1
# dispatch:eval_class end
# dispatch:closure_class
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label106
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label106:
	lw	$t1	8($a0)
	lw	$t1	60($t1)
	jalr	$t1
# dispatch:closure_class end
# dispatch:out_string
# push arg :0
	la	$a0 str_const52
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label107
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label107:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:out_string
# push arg :0
	la	$a0 str_const53
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label108
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label108:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:gen_code
# push arg :0
	la	$a0 VarList_protObj
	jal	Object.copy
	jal	VarList_init
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	12($fp)
	bne	$a0 $zero label109
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label109:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:gen_code end
# dispatch:out_string
# push arg :0
	la	$a0 str_const54
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label110
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label110:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# loop class
label111:
# dispatch:isNil
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label113
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label113:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:isNil end
	lw	$t1	12($a0)
	la	$a0	bool_const1
	beqz	$t1 label114
	la	$a0	bool_const0
label114:
	lw	$t1	12($a0)
	beqz	$t1 label112
# dispatch:headE
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label115
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label115:
	lw	$t1	8($a0)
	lw	$t1	16($t1)
	jalr	$t1
# dispatch:headE end
	sw	$a0	-8($fp)
# dispatch:headC
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label116
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label116:
	lw	$t1	8($a0)
	lw	$t1	20($t1)
	jalr	$t1
# dispatch:headC end
	sw	$a0	-12($fp)
# dispatch:headN
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label117
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label117:
	lw	$t1	8($a0)
	lw	$t1	24($t1)
	jalr	$t1
# dispatch:headN end
	sw	$a0	-16($fp)
# dispatch:removeHead
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label118
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label118:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:removeHead end
# dispatch:gen_closure_code
# push arg :0
	lw	$a0	-16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-8($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :2
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-12($fp)
	bne	$a0 $zero label119
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label119:
	lw	$t1	8($a0)
	lw	$t1	52($t1)
	jalr	$t1
# dispatch:gen_closure_code end
	b	label111
label112:
	li	$a0 0
# loop class end
# dispatch:out_string
# push arg :0
	la	$a0 str_const50
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label120
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label120:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$fp	28($sp)
	lw	$s0	24($sp)
	lw	$ra	20($sp)
	addiu	$sp	$sp	28
	addiu	$sp	$sp	4
	jr	$ra

Main.main:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:print_self
# evaluate dispatch expr
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label121
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label121:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	bne	$a0 $zero label122
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label122:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const1
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label123
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label123:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label124
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label124:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	bne	$a0 $zero label125
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label125:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const1
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label126
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label126:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label127
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label127:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	bne	$a0 $zero label128
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label128:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const1
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label129
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label129:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:beta_reduce
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label130
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label130:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label131
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label131:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label132
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label132:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label133
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label133:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label134
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label134:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label135
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label135:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label136
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label136:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label137
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label137:
	lw	$t1	8($a0)
	lw	$t1	52($t1)
	jalr	$t1
# dispatch:beta_reduce end
# dispatch:beta_reduce
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label138
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label138:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label139
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label139:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label140
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label140:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label141
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label141:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label142
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label142:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label143
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label143:
	lw	$t1	8($a0)
	lw	$t1	52($t1)
	jalr	$t1
# dispatch:beta_reduce end
# dispatch:gen_code
# push arg :0
# dispatch:app
# push arg :0
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label144
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label144:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label145
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label145:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label146
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label146:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label147
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label147:
	lw	$t1	8($a0)
	lw	$t1	64($t1)
	jalr	$t1
# dispatch:gen_code end
# dispatch:gen_code
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label148
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label148:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label149
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label149:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label150
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label150:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label151
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label151:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label152
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label152:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label153
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label153:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label154
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label154:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label155
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label155:
	lw	$t1	8($a0)
	lw	$t1	64($t1)
	jalr	$t1
# dispatch:gen_code end
# dispatch:gen_code
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label156
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label156:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label157
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label157:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label158
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label158:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label159
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label159:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label160
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label160:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label161
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label161:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label162
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label162:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label163
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label163:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label164
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label164:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label165
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label165:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label166
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label166:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label167
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label167:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label168
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label168:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label169
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label169:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label170
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label170:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label171
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label171:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label172
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label172:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label173
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label173:
	lw	$t1	8($a0)
	lw	$t1	64($t1)
	jalr	$t1
# dispatch:gen_code end
# dispatch:gen_code
# push arg :0
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
# dispatch:i
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label174
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label174:
	lw	$t1	8($a0)
	lw	$t1	40($t1)
	jalr	$t1
# dispatch:i end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:app
# push arg :0
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label175
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label175:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label176
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label176:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label177
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label177:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label178
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label178:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:app
# push arg :0
# dispatch:k
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label179
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label179:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:k end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:app
# push arg :0
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label180
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label180:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:s
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label181
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label181:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:s end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label182
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label182:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label183
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label183:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label184
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label184:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label185
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label185:
	lw	$t1	8($a0)
	lw	$t1	64($t1)
	jalr	$t1
# dispatch:gen_code end
	la	$a0 int_const1
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Term.var:
	addiu	$sp	$sp	-16
	sw	$fp	16($sp)
	sw	$s0	12($sp)
	sw	$ra	8($sp)
	addiu	$fp	$sp	8
	move	$s0	$a0
	la	$a0 Variable_protObj
	jal	Object.copy
	jal	Variable_init
	sw	$a0	-4($fp)
# dispatch:init
# push arg :0
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label186
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label186:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:init end
	lw	$fp	16($sp)
	lw	$s0	12($sp)
	lw	$ra	8($sp)
	addiu	$sp	$sp	16
	addiu	$sp	$sp	4
	jr	$ra

Term.lam:
	addiu	$sp	$sp	-16
	sw	$fp	16($sp)
	sw	$s0	12($sp)
	sw	$ra	8($sp)
	addiu	$fp	$sp	8
	move	$s0	$a0
	la	$a0 Lambda_protObj
	jal	Object.copy
	jal	Lambda_init
	sw	$a0	-4($fp)
# dispatch:init
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label187
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label187:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:init end
	lw	$fp	16($sp)
	lw	$s0	12($sp)
	lw	$ra	8($sp)
	addiu	$sp	$sp	16
	addiu	$sp	$sp	8
	jr	$ra

Term.app:
	addiu	$sp	$sp	-16
	sw	$fp	16($sp)
	sw	$s0	12($sp)
	sw	$ra	8($sp)
	addiu	$fp	$sp	8
	move	$s0	$a0
	la	$a0 App_protObj
	jal	Object.copy
	jal	App_init
	sw	$a0	-4($fp)
# dispatch:init
# push arg :0
	lw	$a0	16($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label188
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label188:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:init end
	lw	$fp	16($sp)
	lw	$s0	12($sp)
	lw	$ra	8($sp)
	addiu	$sp	$sp	16
	addiu	$sp	$sp	8
	jr	$ra

Term.i:
	addiu	$sp	$sp	-16
	sw	$fp	16($sp)
	sw	$s0	12($sp)
	sw	$ra	8($sp)
	addiu	$fp	$sp	8
	move	$s0	$a0
# dispatch:var
# push arg :0
	la	$a0 str_const35
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label189
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label189:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:var end
	sw	$a0	-4($fp)
# dispatch:lam
# push arg :0
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label190
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label190:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:lam end
	lw	$fp	16($sp)
	lw	$s0	12($sp)
	lw	$ra	8($sp)
	addiu	$sp	$sp	16
	addiu	$sp	$sp	0
	jr	$ra

Term.k:
	addiu	$sp	$sp	-20
	sw	$fp	20($sp)
	sw	$s0	16($sp)
	sw	$ra	12($sp)
	addiu	$fp	$sp	12
	move	$s0	$a0
# dispatch:var
# push arg :0
	la	$a0 str_const35
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label191
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label191:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:var end
	sw	$a0	-4($fp)
# dispatch:var
# push arg :0
	la	$a0 str_const36
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label192
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label192:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:var end
	sw	$a0	-8($fp)
# dispatch:lam
# push arg :0
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:lam
# push arg :0
	lw	$a0	-8($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label193
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label193:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:lam end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label194
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label194:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:lam end
	lw	$fp	20($sp)
	lw	$s0	16($sp)
	lw	$ra	12($sp)
	addiu	$sp	$sp	20
	addiu	$sp	$sp	0
	jr	$ra

Term.s:
	addiu	$sp	$sp	-24
	sw	$fp	24($sp)
	sw	$s0	20($sp)
	sw	$ra	16($sp)
	addiu	$fp	$sp	16
	move	$s0	$a0
# dispatch:var
# push arg :0
	la	$a0 str_const35
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label195
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label195:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:var end
	sw	$a0	-4($fp)
# dispatch:var
# push arg :0
	la	$a0 str_const36
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label196
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label196:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:var end
	sw	$a0	-8($fp)
# dispatch:var
# push arg :0
	la	$a0 str_const37
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label197
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label197:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:var end
	sw	$a0	-12($fp)
# dispatch:lam
# push arg :0
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:lam
# push arg :0
	lw	$a0	-8($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:lam
# push arg :0
	lw	$a0	-12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:app
# push arg :0
# dispatch:app
# push arg :0
	lw	$a0	-4($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label198
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label198:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
# dispatch:app
# push arg :0
	lw	$a0	-8($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	lw	$a0	-12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label199
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label199:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label200
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label200:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:app end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label201
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label201:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:lam end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label202
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label202:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:lam end
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label203
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label203:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:lam end
	lw	$fp	24($sp)
	lw	$s0	20($sp)
	lw	$ra	16($sp)
	addiu	$sp	$sp	24
	addiu	$sp	$sp	0
	jr	$ra

VarList.isNil:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	la	$a0 bool_const1
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

VarList.head:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label204
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label204:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	la	$a0 Variable_protObj
	jal	Object.copy
	jal	Variable_init
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

VarList.tail:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label205
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label205:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	la	$a0 VarList_protObj
	jal	Object.copy
	jal	VarList_init
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

VarList.add:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:init
# push arg :0
	lw	$a0	12($fp)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# push arg :1
	move	$a0	$s0
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	la	$a0 VarListNE_protObj
	jal	Object.copy
	jal	VarListNE_init
	bne	$a0 $zero label206
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label206:
	lw	$t1	8($a0)
	lw	$t1	48($t1)
	jalr	$t1
# dispatch:init end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	4
	jr	$ra

VarList.print:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	la	$a0 str_const1
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label207
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label207:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

VarListNE.isNil:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	la	$a0 bool_const0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

VarListNE.head:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	12($s0)
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

VarListNE.tail:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	16($s0)
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

VarListNE.init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	16($fp)
	sw	$a0	12($s0)
	lw	$a0	12($fp)
	sw	$a0	16($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	8
	jr	$ra

VarListNE.print:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:print_self
# evaluate dispatch expr
	lw	$a0	12($s0)
	bne	$a0 $zero label208
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label208:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const2
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label209
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label209:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print
# evaluate dispatch expr
	lw	$a0	16($s0)
	bne	$a0 $zero label210
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label210:
	lw	$t1	8($a0)
	lw	$t1	44($t1)
	jalr	$t1
# dispatch:print end
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Variable.init:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	lw	$a0	12($fp)
	sw	$a0	12($s0)
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	4
	jr	$ra

Variable.print_self:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
# dispatch:out_string
# push arg :0
	lw	$a0	12($s0)
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label211
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label211:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Variable.beta:
	addiu	$sp	$sp	-12
	sw	$fp	12($sp)
	sw	$s0	8($sp)
	sw	$ra	4($sp)
	addiu	$fp	$sp	4
	move	$s0	$a0
	move	$a0	$s0
	lw	$fp	12($sp)
	lw	$s0	8($sp)
	lw	$ra	4($sp)
	addiu	$sp	$sp	12
	addiu	$sp	$sp	0
	jr	$ra

Variable.substitute:
	addiu	$sp	$sp	-20
	sw	$fp	20($sp)
	sw	$s0	16($sp)
	sw	$ra	12($sp)
	addiu	$fp	$sp	12
	move	$s0	$a0
	lw	$a0	16($fp)
	sw	$a0	-4($fp)
	move	$a0	$s0
	sw	$a0	-8($fp)
	la	$a0	bool_const1
	lw	$t1	-4($fp)
	lw	$t2	-8($fp)
	beq	$t1 $t2 label212
	la	$a1	bool_const0
	jal	equality_test
label212:
	lw	$t1	12($a0)
	beqz	$t1 label213
	lw	$a0	12($fp)
	b	label214
label213:
	move	$a0	$s0
label214:
	lw	$fp	20($sp)
	lw	$s0	16($sp)
	lw	$ra	12($sp)
	addiu	$sp	$sp	20
	addiu	$sp	$sp	8
	jr	$ra

Variable.gen_code:
	addiu	$sp	$sp	-24
	sw	$fp	24($sp)
	sw	$s0	20($sp)
	sw	$ra	16($sp)
	addiu	$fp	$sp	16
	move	$s0	$a0
	lw	$a0	16($fp)
	sw	$a0	-4($fp)
# loop class
label215:
# dispatch:isNil
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label217
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label217:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:isNil end
	lw	$t1	12($a0)
	beqz	$t1 label218
	la	$a0 bool_const0
	b	label219
label218:
# dispatch:head
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label220
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label220:
	lw	$t1	8($a0)
	lw	$t1	32($t1)
	jalr	$t1
# dispatch:head end
	sw	$a0	-8($fp)
	move	$a0	$s0
	sw	$a0	-12($fp)
	la	$a0	bool_const1
	lw	$t1	-8($fp)
	lw	$t2	-12($fp)
	beq	$t1 $t2 label221
	la	$a1	bool_const0
	jal	equality_test
label221:
	lw	$t1	12($a0)
	la	$a0	bool_const1
	beqz	$t1 label222
	la	$a0	bool_const0
label222:
label219:
	lw	$t1	12($a0)
	beqz	$t1 label216
# dispatch:out_string
# push arg :0
	la	$a0 str_const7
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label223
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label223:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:tail
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label224
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label224:
	lw	$t1	8($a0)
	lw	$t1	36($t1)
	jalr	$t1
# dispatch:tail end
	sw	$a0	-4($fp)
	b	label215
label216:
	li	$a0 0
# loop class end
# dispatch:isNil
# evaluate dispatch expr
	lw	$a0	-4($fp)
	bne	$a0 $zero label225
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label225:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:isNil end
	lw	$t1	12($a0)
	beqz	$t1 label226
# dispatch:out_string
# push arg :0
	la	$a0 str_const8
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label228
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label228:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:print_self
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label229
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label229:
	lw	$t1	8($a0)
	lw	$t1	28($t1)
	jalr	$t1
# dispatch:print_self end
# dispatch:out_string
# push arg :0
	la	$a0 str_const1
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label230
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label230:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
# dispatch:abort
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label231
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label231:
	lw	$t1	8($a0)
	lw	$t1	0($t1)
	jalr	$t1
# dispatch:abort end
	move	$a0	$s0
	b	label227
label226:
# dispatch:out_string
# push arg :0
	la	$a0 str_const9
	sw	$a0	0($sp)
	addiu	$sp	$sp	-4
# evaluate dispatch expr
	move	$a0	$s0
	bne	$a0 $zero label232
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label232:
	lw	$t1	8($a0)
	lw	$t1	12($t1)
	jalr	$t1
# dispatch:out_string end
label227:
	lw	$fp	24($sp)
	lw	$s0	20($sp)
	lw	$ra	16($sp)
	addiu	$sp	$sp	24
	addiu	$sp	$sp	8
	jr	$ra


# end of generated code
